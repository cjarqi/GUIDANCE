...
from .models import ViolationCatalog, Program, StaffUser, UserRole
...
def seed_database(secret_key):
    # IMPORTANT: Use a secret key you will remember or set it as an environment variable.
    SEEDING_SECRET = os.environ.get('SEEDING_SECRET', 'guidance123') # Using the key from your logs
    ...
    try:
        ...
        if StaffUser.query.filter_by(username="superadmin").first():
            output += "Superadmin user already exists. Skipping.\n"
        else:
            ...
            output += "Superadmin user created.\n"
        ...
        output += "\nChecking and creating programs...\n"
        for p_data in programs_data:
            if not Program.query.filter_by(program_code=p_data["code"]).first():
                prog = Program(program_name=p_data["name"], program_code=p_data["code"])
                db.session.add(prog)
                output += f"  - Created program: {p_data['code']}\n"

        # --- 3. Create FULL Violation Catalog ---
        output += "\nChecking and creating violations...\n"
        violations_data = [
        ...
        for v_data in violations_data:
            # This check prevents creating duplicates if you run the seeder more than once
            if not ViolationCatalog.query.filter_by(violation_code=v_data["code"]).first():
                v = ViolationCatalog(
                    violation_code=v_data["code"],
                    description=v_data["desc"],
                    type=v_data["type"],
                    is_inherently_major=v_data.get("is_major", False),
                    reference_page=v_data.get("ref")
                )
                db.session.add(v)
                output += f"  - Created violation: {v_data['code']}\n"
